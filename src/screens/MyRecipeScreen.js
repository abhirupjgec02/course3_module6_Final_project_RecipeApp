import {
  ...
  const addRecipe = async (recipe) => {
    setrecipes((prev) => {
      ...
      AsyncStorage.setItem("customrecipes", JSON.stringify(updated));
      return updated; // ✅ updates UI immediately
    });
  };
  
  const handleAddrecipe = () => {
    navigation.navigate("RecipesFormScreen", {onrecipeAdded : addRecipe});
  };
  ...
  const editrecipe = (recipe, index) => {
    navigation.navigate("RecipesFormScreen", { recipeToEdit: recipe, recipeIndex: index, onrecipeEdited: onEdited});
  };
